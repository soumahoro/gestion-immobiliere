...
import { EntityArrayResponseType, ProprietaireService } from '../service/proprietaire.service';
...
export class ProprietaireComponent implements OnInit {
  ...
  proprietaires = signal<IProprietaire[]>([]);
  isLoading = false;
  ...
  protected readonly proprietaireService = inject(ProprietaireService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IProprietaire): number => this.proprietaireService.getProprietaireIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.proprietaires.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IProprietaire[] | null): IProprietaire[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.proprietaireService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
