...
import { AnneeService, EntityArrayResponseType } from '../service/annee.service';
...
export class AnneeComponent implements OnInit {
  ...
  protected readonly anneeService = inject(AnneeService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IAnnee): number => this.anneeService.getAnneeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.annees.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IAnnee[] | null): IAnnee[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.anneeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
